...
sub build_data (){
	...
	$sth = $dbh->prepare("SELECT state,akills FROM $pilot_mis_tbl WHERE hlname=? order by misrep ASC");
	$sth->execute($hlname);
...
#    print GP "set yrange [0:".($max_mis+3)."]\n";
    ...
    print GP "set xlabel \"Missions\"\n";
    print GP "set ylabel \"Airkills\"\n";
...
#    print GP "plot \"$PD/$ak_x_life\" title \"Misiones por vida \", \"$PD/$derribos\" title \"Derribos por vida \", \"$PD/$objtierra\" title \"Obj. Terrestres por vida \"\nquit\n";
    ...
    print GP "plot ";
    print GP " x/2    t\"0,5 * mis\"  w l lt 3 , ";
    print GP " x      t\"1,0 * mis\"  w l lt 4 , ";

    if ($max_scale>1) {print GP " x*1.5  t\"1,5 * mis\"  w l lt 3 , ";}
    if ($max_scale>1.5) {print GP " x*2  t\"2,0 * mis\"  w l lt 3 , ";}

    print GP " \"$PD/$ak_x_life\" title \"Airkills per live (max $max_ak) \" w l lt 1, ";
    print GP " \"$PD/$derribos\"  title \"Total airkills ($akills)\" w l lt 2 \n  ";
    print GP "quit\n";
